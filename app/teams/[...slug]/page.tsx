...
import { GET_TEAM_BY_PATH } from '@/lib/queries'
import { DrupalTeam } from '@/lib/types'
...
interface TeamByPathData {
  route: {
    entity: DrupalTeam
  } | null
}

async function getTeam(path: string): Promise<DrupalTeam | null> {
  try {
    const requestHeaders = await headers()
    const apolloClient = getServerApolloClient(requestHeaders)
    const { data } = await apolloClient.query<TeamByPathData>({
      query: GET_TEAM_BY_PATH,
      variables: { path },
...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  ...
  const path = `/teams/${slug.join('/')}`
  const item = await getTeam(path)

  if (!item) {
    return { title: 'Team Not Found | Sports League' }
  }
            ...
            <div className="lg:col-span-1">
              <div className="bg-white rounded-xl shadow-sm p-6 sticky top-24">
                ...
                <dl className="space-y-4">
                  {(item as any).coach && (
                    <div>
                      <dt className="text-sm text-gray-500">Head Coach</dt>
                      <dd className="font-semibold text-gray-900">{(item as any).coach}</dd>
                    </div>
                  )}
                </dl>
                <div className="mt-8">
