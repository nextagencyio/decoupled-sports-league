...
async function getTeams() {
  try {
    ...
    const { data } = await apolloClient.query<TeamsData>({ query: GET_TEAMS, variables: { first: 50 }, fetchPolicy: 'cache-first' })
      ...
      <section className="py-16 md:py-20">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          {items.length === 0 ? (
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">{items.map((item) => (<TeamCard key={item.id} item={item} />))}</div>
          )}
        </div>
      </section>
    </div>
