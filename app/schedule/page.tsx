...
export const metadata: Metadata = {
  ...
  description: 'View upcoming games, matchups, venues, and results for Pacific Athletic League.',
}
...
export default async function ScheduleEntriesPage() {
  const items = await getScheduleEntries()
  return (
    <div className="min-h-screen bg-gray-50">
      ...
      <section className="relative bg-gradient-to-r from-primary-800 via-primary-900 to-primary-800 text-white py-16 md:py-24 overflow-hidden">
        ...
        <div className="relative z-10 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="text-center">
            <h1 className="font-display text-4xl md:text-5xl lg:text-6xl font-bold mb-6 uppercase">Game Schedule</h1>
            <p className="text-xl text-primary-200 max-w-3xl mx-auto">Upcoming matchups, venues, and results for the current season.</p>
          </div>
        </div>
      </section>
      <section className="py-16 md:py-20">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          {items.length === 0 ? (
            <div className="text-center py-12"><h2 className="text-2xl font-semibold text-gray-600 mb-2">No Games Scheduled Yet</h2><p className="text-gray-500">Game schedule will appear here once content is imported.</p></div>
          ) : (
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">{items.map((item: any) => (<ScheduleEntryCard key={item.id} item={item} />))}</div>
          )}
        </div>
      </section>
    </div>
