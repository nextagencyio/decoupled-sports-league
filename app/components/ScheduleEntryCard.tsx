...
interface ScheduleEntryCardProps {
  item: DrupalScheduleEntry
}

export default function ScheduleEntryCard({ item }: ScheduleEntryCardProps) {
  return (
    <Link href={item.path || '#'} className="group bg-white rounded-xl shadow-md hover:shadow-lg transition-all duration-300 overflow-hidden">
      <div className="relative h-48 bg-gradient-to-br from-primary-600 to-accent-800">
        {(item as any).image?.url ? (
          <ResponsiveImage src={(item as any).image.url} alt={(item as any).image.alt || item.title} fill
            className="object-cover group-hover:scale-105 transition-transform duration-300"
            variations={(item as any).image.variations} targetWidth={400} />
        ) : (
      ...
      <div className="p-6">
        {(item as any).homeTeam && (
          <p className="text-sm text-primary-700 font-medium mb-2">{(item as any).homeTeam} vs {(item as any).awayTeam}</p>
        )}
