...
import { GET_FEATURED_TEAMS } from '@/lib/queries'
...
interface FeaturedTeamsData {
  nodeTeams: {
    nodes: DrupalTeam[]
  }
}

export default function TeamsPreview({ homepageContent }: TeamsPreviewProps) {
  const { data, loading, error } = useQuery<FeaturedTeamsData>(GET_FEATURED_TEAMS)

  const teams = data?.nodeTeams?.nodes || []
