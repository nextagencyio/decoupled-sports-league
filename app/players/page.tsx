...
async function getPlayers() {
  try {
    ...
    const data = await client.raw(GET_PLAYERS, { first: 50 })
      ...
      <section className="py-16 md:py-20">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          {items.length === 0 ? (
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">{items.map((item: any) => (<PlayerCard key={item.id} item={item} />))}</div>
          )}
        </div>
      </section>
    </div>
